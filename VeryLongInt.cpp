...
VeryLongInt VeryLongInt::operator/(const VeryLongInt &other) const {
	...
	if (dividend.negative != other.negative) {
		negativeResult = true; // 根据被除数和除数的符号确定结果的符号
	}

	dividend.negative = false;
	VeryLongInt divisorPositive = other;
	divisorPositive.negative = false; // 确保divisorPositive始终为正数
	...
	for (int i = dividend.digits.size() - 1; i >= 0; --i) {
		...
		while (remainder >= divisorPositive) {
			remainder = remainder - divisorPositive; // 通过从余数中减去divisorPositive执行除法步骤
	...
	quotient.negative = negativeResult; // 设置商的符号
	return quotient; // 返回商
}
...
VeryLongInt VeryLongInt::operator%(const VeryLongInt &other) const {
	...
	// 将被除数转换为绝对值，并将其存储在临时变量absDividend中
	VeryLongInt absDividend = *this;
	absDividend.negative = false;

	// 将除数转换为绝对值，并将其存储在临时变量absDivisor中
